...
public class player19 implements ContestSubmission {
	...
	private void golden() {
		...
		int pop = limit_ - 1;// population_/2 - 1;
		for (int i = 0; i < DIM; i++)
...
//			System.out.println(lambda);
	...
	private void CMA_ES(int lambda, double sigma) {
		// Set parameters
		...
		for (int g = 0;; g++) {
			...
			*/
			...
			if (g >= 50 && best_score[g] - best_score[g - 30] < endDiff) {
				//break;
			}
	...
	private void DE() {
		...
		for (int i = 0; i < population; i++)
			score[i] = Math.log((Double) evaluation_.evaluate(g[i]));
